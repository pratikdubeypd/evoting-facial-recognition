...
import json, os, face_recognition, cv2
...
def publicPolls(request):
    if request.user.is_authenticated:
        ...
        if 'search' in request.GET:
            ...
            allPublicPollstitle = allPublicPolls.filter(title__icontains=search_term)
...
def privatePolls(request):
    if request.user.is_authenticated:
        ...
        if allPrivatePolls is not None:
            ...
            if 'search' in request.GET:
                search_term = request.GET['search']
                allPrivatePollstitle = allPrivatePolls.filter(title__icontains=search_term)
                allPrivatePollsdesc =allPrivatePolls.filter(desc__icontains=search_term)
                allPrivatePolls =allPrivatePollstitle.union(allPrivatePollsdesc)
            dict = {'allPrivatePolls':allPrivatePolls, 'search_term':search_term}
...
def publicrefresh():
    ...
    outdatedpoll = Publicpoll.objects.raw("select * from polls_Publicpoll where endtime < %s and isActive = True", [now])
...
def inviterefresh():
    now = datetime.datetime.now()
    invitations = Privateinvite.objects.raw("select * from polls_Privateinvite where expiry < %s and isActive = True", [now])
...
def privaterefresh():
    now = datetime.datetime.now()
    outdatedpoll = Privatepoll.objects.raw("select * from polls_Privatepoll where endtime < %s and isActive = True", [now])
    outdatedpoll = list(outdatedpoll)
    for i in range(len(outdatedpoll)):
        outdatedpoll[i].isActive = False
        outdatedpoll[i].save()
...
def createPublicPoll(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if len(title) > 200:
                messages.error(request, 'Your title must be under 200 characters')
                return render(request, 'createpolls.html')
            if len(desc) > 500:
                messages.error(request, 'Your description must be under 500 characters')
                return render(request, 'createpolls.html')
            if len(choice1) > 300:
            ...
            if Publicpoll.objects.filter(title=title).exists():
                if Publicpoll.objects.filter(desc=desc).exists():
                    ...
                    return render(request, 'createpolls.html')
            publicpoll = Publicpoll(owner=owner,title=title,desc=desc,isActive=True,endtime=endtime,choice1=choice1,choice2=choice2,genre=genre)
            publicpoll.save()
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if len(choice2) > 300:
                messages.error(request, 'Your 2nd choice is too long')
                return render(request, 'createpolls.html')
            privatepoll = Privatepoll(owner=owner,title=title,desc=desc,isActive=True,endtime=endtime,choice1=choice1,choice2=choice2,genre=genre)
            privatepoll.save()
            ...
            if delete == "delete":
                ...
                if publicpoll is not None:
                    ...
                    if request.user == publicpoll.owner:
                        ...
                        publicpoll.save()
                        messages.success(request, 'The public poll is deleted successfully!')
                        return redirect('publicpolls')
                    else:
...
def privateinvite(request, poll_id):
    if request.user.is_authenticated:
        ...
        if privatepoll is not None:
            ...
            return render(request, 'privateinvite.html', {'usernames':usernames, 'privatepoll':privatepoll, 'privateinvites':privateinvites})
        else:
            messages.error(request, 'No such poll exists :(')
            return redirect('privatepolls')
    else:
            ...
            if deleteinvite == "reject":
                ...
                if privatepoll is not None:
                    if request.user == privatepoll.owner:
                        ...
                        inviterefresh()
                        privateinvite = Privateinvite.objects.filter(userinvited = reuser, poll = privatepoll, is_accepted = False).first()
        ...
        if privatepoll is not None:
            ...
            if privatepoll.owner == request.user or hasAccess == True:
                if privatepoll.isActive:
                    ...
                    return render(request, 'privatepolldetails.html', {'poll':privatepoll, 'hasVoted':hasVoted, 'hasAccess':hasAccess})
                else:
                    messages.error(request, 'The poll is not active anymore :(')
                    return redirect('privatepolls')
            else:
            ...
            if privatepoll is not None:
                ...
                if facedetect(requser.userprofile.head_shot.url):
                    if Privatevote.objects.filter(user=request.user, poll=privatepoll).exists():
                        ...
                        choice = request.POST['choice']
                        privatevote = Privatevote(user = request.user, poll = privatepoll, choice = choice)
                        privatevote.save()
                        ...
                        privatepoll.save()
                        messages.success(request, 'Facial authentication was successful and you have successfully voted!')
                        return redirect('privatepolldetails', poll_id=poll_id)
                else:
            ...
            if privatepoll is not None:
                if privatepoll.owner == request.user:
                    ...
                    if Privateinvite.objects.filter(userinvited=userin, poll=privatepoll, isActive = True).exists():
                        messages.warning(request, 'This user is already invited!')
                        return redirect('privateinvite', poll_id=poll_id)
                    else:
                        expiry = datetime.datetime.now() + datetime.timedelta(days=7)
                        privateinvite = Privateinvite(userinvited = userin, poll = privatepoll, expiry = expiry)
                        privateinvite.save()
                        # send userin some notification
                        messages.success(request, 'Invitation sent successfully!')
                        messages.warning(request, 'Note: 1. This invitation will be revoked after 7 days. 2. If the user accepts the invitation, it can\'t be revoked.')
                        return redirect('privateinvite', poll_id=poll_id)
                else:
                    messages.error(request, 'You do not have the permission to invite anyone to this poll!')
                    return redirect('privatepolls')
            else:
                messages.error(request, 'No such poll exists :(')
                return redirect('privatepolls')
                ...
                if privatepoll.isActive:
                    if invitechoice == "accept":
                        if Privateinvite.objects.filter(userinvited = request.user, poll = privatepoll, isActive = True).exists():
                            privateinvite = Privateinvite.objects.filter(userinvited = request.user, poll = privatepoll).first()
                        ...
                        if Privateinvite.objects.filter(userinvited = request.user, poll = privatepoll).exists():
                            privateinvite = Privateinvite.objects.filter(userinvited = request.user, poll = privatepoll).first()
                            privateinvite.isActive = False
                            privateinvite.save()
    ...
    if request.user.is_authenticated:
        ...
        if publicpoll is not None:
            if publicpoll.isActive:
                publicvote = Publicvote.objects.filter(user=request.user, poll=publicpoll).first()
                hasVoted = Publicvote.objects.filter(user=request.user, poll=publicpoll).exists()
                dict = {'publicpoll':publicpoll,'publicvote':publicvote,'hasVoted':hasVoted}
    ...
    if request.user.is_authenticated:
        ...
        if privatepoll is not None:
            if privatepoll.isActive:
                ...
                privatevote = Privatevote.objects.filter(user=request.user, poll=privatepoll).first()
                hasVoted = Privatevote.objects.filter(user=request.user, poll=privatepoll).exists()
                dict = {'privatepoll':privatepoll,'privatevote':privatevote,'hasVoted':hasVoted, 'hasAccess':hasAccess}
